...
math <- read.csv("Raw_Datasets/OECD_TestScores/OECD_MathScores.csv")
reading <- read.csv("Raw_Datasets/OECD_TestScores/OECD_ReadingScores.csv")
science <- read.csv("Raw_Datasets/OECD_TestScores/OECD_ScienceScores.csv")
...
## Add a new column that shows the total average score per country, gender, and year
all_subject_1 <- all_subject %>%
  group_by(country_code, gender, year) %>%
  mutate(
    sum_total_score = sum(test_score)
  ) %>%
  ungroup()

## Create a new column to rank the countries by total test score for each year
all_subject_1 <- all_subject_1 %>%
  dplyr::arrange(year, sum_total_score) %>%
  dplyr::group_by(gender, year) %>%
  dplyr::mutate(rank = dense_rank(desc(sum_total_score)))

## Write data frame to csv file
write_csv(all_subject_1, "Clean_Datasets/OECD_Test_Scores_Clean.csv")
